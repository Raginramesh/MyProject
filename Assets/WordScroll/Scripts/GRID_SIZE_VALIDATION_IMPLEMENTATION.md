...
### Blank Cell Strategy
- **Leading or Trailing Blanks Only**: `[_][C][A][T][S]` ✅ Valid, `[C][A][T][S][_]` ✅ Valid
- **No Middle Blanks**: `[W][O][_][R][D]` ❌ Invalid
- **Dictionary Lookup**: Blanks ignored, only letter part validated
  - `[_][C][A][T][S]` validates "CATS" in dictionary
  - `[C][A][T][S][_]` validates "CATS" in dictionary
  - `[_][_][C][A][T]` validates "CAT" in dictionary
...
- `HasValidSequenceStructure()`: Ensures blanks only appear at beginning or end of sequences, never in middle
- `ExtractWordFromSequence()`: Strips leading and trailing blanks for dictionary lookup
...
[C][A][T][S][_] → Valid (CATS + 1 trailing blank = 5 cells)
[_][C][A][T][S] → Valid (CATS + 1 leading blank = 5 cells)
[_][_][C][A][T] → Valid (CAT + 2 leading blanks = 5 cells)
[C][A][T][_][_] → Valid (CAT + 2 trailing blanks = 5 cells)
[W][O][_][R][D] → Invalid (blank in middle)
[C][A][T]       → Invalid (only 3 cells in 5x5 grid)

// 3x3 Grid Examples  
[I][T][_] → Valid (IT + 1 trailing blank = 3 cells)
[_][I][T] → Valid (IT + 1 leading blank = 3 cells)
